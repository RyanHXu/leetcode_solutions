...
class AllTaskSchedulingOrders {
 ...
 private:
  static void printAllTopologicalSorts(unordered_map<int, vector<int>> &graph,
    ...
    if (!sources.empty()) {
      for (int vertex : sources) {
        ...
        for (auto child : children) {
          inDegree[child]--;
          if (inDegree[child] == 0) {
            sourcesForNextCall.push_back(
                child);  // save the new source for the next call
          }
        }

        // recursive call to print other orderings from the remaining (and new)
        // sources
        printAllTopologicalSorts(graph, inDegree, sourcesForNextCall,
                                 sortedOrder);

        // backtrack, remove the vertex from the sorted order and put all of its
        // children back to consider the next source instead of the current
        // vertex
    ...
    // if sortedOrder doesn't contain all tasks, either we've a cyclic
    // dependency between tasks, or we have not processed all the tasks in this
    // recursive call
