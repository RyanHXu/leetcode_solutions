...
def solve_knapsack(profits, weights, capacity):
    ...
    def knapsackTopdown(profits,weights,capacity,index=0):
        if index>n-1 or capacity<=0:
            return 0
        ...
        if weights[index]<=capacity:
            p1 = profits[index] + knapsackTopdown(profits,weights,capacity-weights[index],index+1)
        p2 = knapsackTopdown(profits,weights,capacity,index+1)
