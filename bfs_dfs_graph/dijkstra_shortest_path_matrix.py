...
class Graph():
    ...
    def dijkstra(self, src):
        ...
        for cout in range(self.V):
            ...
            # get index of minimum distance vertex in the following order
            # 0 1 7 6 5 2 8 3 4
            ...
            for y in range(self.V):
                # if self.graph[x][y] > 0, it means there is an edge from x to y
                # if sptSet[y] is False, it means y is not yet included in the shortest path tree
                if self.graph[x][y] > 0 and sptSet[y] == False:
                    count +=1
                    if dist[y] > dist[x] + self.graph[x][y]:
